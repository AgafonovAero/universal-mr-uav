...
%
% Units:
%   q_nb [-], euler_rpy_rad [rad], gyro_b_radps [rad/s]
%
% Assumptions:
...
mag_b_uT = local_validate_vec3(sens, {'mag', 'field_b_uT'}, ...
    'sens.mag.field_b_uT');
...
if ~isstruct(est_prev) || ~isscalar(est_prev)
    error('uav:est:attitude_cf_step:StateType', ...
        'Expected est_prev to be a scalar struct.');
end

est_prev.q_nb = local_validate_numeric_vec(est_prev, 'q_nb', 4);
est_prev.euler_rpy_rad = local_validate_numeric_vec(est_prev, ...
    'euler_rpy_rad', 3);
...
if ~isstruct(params) || ~isfield(params, 'estimator') || ...
    ...
    error('uav:est:attitude_cf_step:MissingParams', ...
...
for k = 1:numel(fields)
    ...
    if ~isstruct(value) || ~isfield(value, field_name)
        error('uav:est:attitude_cf_step:MissingField', ...
...
if ~isfield(data, field_name)
    error('uav:est:attitude_cf_step:MissingStateField', ...
...
q_dot = 0.5 .* local_quat_multiply(q_prev, [0.0; gyro_b_radps]);
...
pitch_rad = asin(max(min(accel_b_mps2(1) ./ accel_norm, 1.0), -1.0));
is_valid = true;
end
...
ref_heading_rad = atan2(params.env.mag_ned_uT(2), params.env.mag_ned_uT(1));
...
yaw_rad = local_wrap_angle_pi(ref_heading_rad - ...
    atan2(mag_level_ned_uT(2), mag_level_ned_uT(1)));
is_valid = true;
end
