function tests = test_run_case_with_sensors
%TEST_RUN_CASE_WITH_SENSORS Tests for the case runner with sensor history.
...
function testHoverCaseReturnsSensorHistoryAndBoundedDrift(testCase)
...
verifyTrue(testCase, isfield(log, 'sensors'));
verifyEqual(testCase, numel(log.sensors), numel(log.time_s));
verifyTrue(testCase, isfield(log.sensors(end), 'imu'));
verifyEqual(testCase, log.quat_norm(end), 1.0, 'AbsTol', 1.0e-12);
verifyLessThan(testCase, norm(final_sens.gnss.pos_ned_m(1:2)), 1.0e-9);
verifyLessThan(testCase, abs(final_sens.gnss.pos_ned_m(3)), 10.0);
end
