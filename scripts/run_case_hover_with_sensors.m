%% RUN_CASE_HOVER_WITH_SENSORS Execute the hover case with sensor logging.
% Description:
%   Runs the deterministic hover case through uav.sim.run_case_with_sensors
%   and prints compact end-of-run diagnostics from the sensor history.
%
% Inputs:
%   none
%
...
final_sens = log.sensors(end);

fprintf('Run-case hover with sensors:\n');
fprintf('  final rotor speeds [rad/s]  : [%.6f %.6f %.6f %.6f]\n', ...
    log.omega_m_radps(end, :));
fprintf('  final specific force [m/s^2]: [%.6f %.6f %.6f]\n', ...
    final_sens.imu.accel_b_mps2);
fprintf('  final baro altitude [m]     : %.6f\n', final_sens.baro.alt_m);
fprintf('  final GNSS position NED [m] : [%.6f %.6f %.6f]\n', ...
    final_sens.gnss.pos_ned_m);
...
assignin('base', 'case_hover_with_sensors_demo', demo);
