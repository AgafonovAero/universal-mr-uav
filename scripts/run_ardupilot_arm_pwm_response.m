%% RUN_ARDUPILOT_ARM_PWM_RESPONSE Сравнить команды ШИМ до и после попытки взведения.
% Назначение:
%   Выполняет непрерывный 20-секундный прогон обмена. Через 10 секунд
%   после начала запускает внешнюю попытку взведения ArduPilot в
%   отдельном процессе WSL, не прерывая поток JSON/UDP от MATLAB-модели.
...
[arm_helper_win, arm_result_win, arm_launcher_win, arm_launch_command] = local_prepare_async_arm_helper(cfg);
...
assignin('base', 'ardupilot_live_backend_update_rate_hz', 120.0);
...
arm_result = local_read_async_arm_result(arm_result_win, launch_status, launch_output);
...
save(fullfile(reports_dir, 'task_16_arm_pwm_response.mat'), 'response');
writetable( ...
    local_make_csv_table(live_result), ...
    fullfile(reports_dir, 'task_16_arm_pwm_response.csv'));
...
function [helper_path_win, result_path_win, launcher_path_win, launch_command] = local_prepare_async_arm_helper(cfg)
%LOCAL_PREPARE_ASYNC_ARM_HELPER Подготовить внешний Python-помощник для взведения.

helper_path_win = [tempname, '.py'];
launcher_path_win = [tempname, '.ps1'];
result_path_win = fullfile(tempdir, 'task16_arm_async_result.json');
helper_path_wsl = local_windows_to_wsl_path(helper_path_win);
result_path_wsl = local_windows_to_wsl_path(result_path_win);
target_loop_rate_hz = 50.0;
...
python_lines(end + 1, 1) = "    'command_text': 'python3 async arm helper over tcp:127.0.0.1:5762',";
...
python_lines(end + 1, 1) = "    'sched_loop_rate_hz': None,";
...
python_lines(end + 1, 1) = "        master.mav.param_set_send(master.target_system, master.target_component, b'SCHED_LOOP_RATE', " + sprintf('%.1f', target_loop_rate_hz) + ", mavutil.mavlink.MAV_PARAM_TYPE_REAL32)";
python_lines(end + 1, 1) = "        param_deadline = time.time() + 5.0";
python_lines(end + 1, 1) = "        while time.time() < param_deadline:";
...
python_lines(end + 1, 1) = "                param_id = str(getattr(msg, 'param_id', '')).replace('\x00', '').strip()";
python_lines(end + 1, 1) = "                if param_id == 'SCHED_LOOP_RATE':";
python_lines(end + 1, 1) = "                    result['sched_loop_rate_hz'] = float(msg.param_value)";
...
launcher_lines(end + 1, 1) = "    'sleep 10; python3 " + helper_path_wsl + "'";
launcher_lines(end + 1, 1) = ")";
...
function arm_result = local_read_async_arm_result(result_path_win, launch_status, launch_output)
%LOCAL_READ_ASYNC_ARM_RESULT Прочитать результат внешней попытки взведения.

deadline = tic;
while toc(deadline) < 15.0
    if isfile(result_path_win)
        break;
    end
    pause(0.2);
...
if isfile(result_path_win)
    ...
    if isempty(parsed.ack_result)
        ...
        arm_result.ack_result = double(parsed.ack_result);
    end
    ...
    if isfield(parsed, 'sched_loop_rate_hz') && ~isempty(parsed.sched_loop_rate_hz)
        arm_result.sched_loop_rate_hz = double(parsed.sched_loop_rate_hz);
    else
        arm_result.sched_loop_rate_hz = nan;
    end
    ...
    arm_result.raw_output = string(launch_output);
end
end
...
mask = live_result.time_s >= interval_s(1) & live_result.time_s < interval_s(2);

phase_summary = struct();
phase_summary.time_s = live_result.time_s(mask);
phase_summary.motor_cmd_radps = live_result.motor_cmd_radps(mask, :);
phase_summary.sitl_output = live_result.sitl_output(mask);
...
if fid < 0
    error('uav:task16:run_ardupilot_arm_pwm_response:OpenFile', ...
