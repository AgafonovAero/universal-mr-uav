%% RUN_SIL_STUB_YAW_STEP Build and run the thin SIL-prep shell in yaw-step mode.
% Description:
%   Rebuilds the minimal SIL-prep Simulink shell, runs the temporary
%   external FCS stub in yaw-step mode, and prints compact end-of-run
%   diagnostics.
%
% Inputs:
%   none
%
% Outputs:
%   sil_stub_yaw_step_demo - assigned in base workspace with parsed results
%
% Units:
...
assignin('base', 'sil_stub_mode', "yaw_step");
...
if exist(model_file, 'file') ~= 2
    error('uav:sl:run_sil_stub_yaw_step:MissingModelFile', ...
...
final_yaw_est_rad = truth_log.estimator.euler_rpy_rad(3);
final_yaw_rate_radps = truth_log.state.w_b_radps(3);
final_altitude_m = -truth_log.state.p_ned_m(3);
...
fprintf('Thin SIL-prep yaw-step diagnostics:\n');
...
sil_stub_yaw_step_demo = struct();
sil_stub_yaw_step_demo.params = params;
sil_stub_yaw_step_demo.dt_s = dt_s;
sil_stub_yaw_step_demo.t_final_s = t_final_s;
sil_stub_yaw_step_demo.state0 = state0;
sil_stub_yaw_step_demo.rates_cfg = rates_cfg;
sil_stub_yaw_step_demo.final_truth = truth_log;
sil_stub_yaw_step_demo.final_diag = diag_log;
sil_stub_yaw_step_demo.final_yaw_est_rad = final_yaw_est_rad;
sil_stub_yaw_step_demo.final_yaw_rate_radps = final_yaw_rate_radps;
sil_stub_yaw_step_demo.final_altitude_m = final_altitude_m;
sil_stub_yaw_step_demo.final_estimated_altitude_m = final_estimated_altitude_m;
sil_stub_yaw_step_demo.final_true_quat_norm = final_true_quat_norm;
sil_stub_yaw_step_demo.final_estimated_quat_norm = final_estimated_quat_norm;

assignin('base', 'sil_stub_yaw_step_demo', sil_stub_yaw_step_demo);
...
if ~isa(signal, 'Simulink.SimulationData.Signal')
    error('uav:sl:run_sil_stub_yaw_step:UnexpectedDatasetElement', ...
...
for k = 1:numel(field_names)
    ...
    elseif isa(field_value, 'timeseries')
        ...
        error('uav:sl:run_sil_stub_yaw_step:UnexpectedLeafType', ...
...
elseif size(data, 2) == n_samples
    ...
    error('uav:sl:run_sil_stub_yaw_step:UnexpectedTimeseriesShape', ...
