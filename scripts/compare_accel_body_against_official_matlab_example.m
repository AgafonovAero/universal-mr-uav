...
for idx = 1:row_count
    ...
    sensors = struct();
    sensors.imu = struct( ...
        'accel_b_mps2', cases(idx).current_accel_b_mps2, ...
    ...
    current_ax(idx) = cases(idx).current_accel_b_mps2(1);
    current_ay(idx) = cases(idx).current_accel_b_mps2(2);
    current_az(idx) = cases(idx).current_accel_b_mps2(3);
    official_ax(idx) = sample.imu.accel_body(1);
    official_ay(idx) = sample.imu.accel_body(2);
    official_az(idx) = sample.imu.accel_body(3);
    delta_norm(idx) = norm(sample.imu.accel_body(:) - cases(idx).current_accel_b_mps2(:));
...
cases = repmat(struct( ...
    ...
    'current_accel_b_mps2', zeros(3, 1)), 5, 1);
...
cases(1).current_accel_b_mps2 = [0.0; 0.0; 0.0];
...
cases(2).current_accel_b_mps2 = cases(2).forces_b_N ./ mass;
...
cases(3).current_accel_b_mps2 = cases(3).forces_b_N ./ mass;
...
cases(4).current_accel_b_mps2 = cases(4).forces_b_N ./ mass;
...
cases(5).current_accel_b_mps2 = cases(5).forces_b_N ./ mass;
end
