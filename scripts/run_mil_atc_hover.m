%% RUN_MIL_ATC_HOVER Build and run the thin ATC MIL shell in hover mode.
% Description:
...
%   for the hover smoke scenario.
%
% Inputs:
%   none
%
% Outputs:
%   mil_atc_hover_demo - assigned in base workspace with parsed results
%
% Units:
...
if exist(model_file, 'file') ~= 2
    error('uav:sl:run_mil_atc_hover:MissingModelFile', ...
...
fprintf('Thin ATC MIL hover diagnostics:\n');
...
fprintf('  final motor cmd [rad/s]          : [%.6f %.6f %.6f %.6f]\n', ...
    final_diag.motor_cmd_radps);
...
mil_atc_hover_demo = struct();
mil_atc_hover_demo.params = params;
mil_atc_hover_demo.dt_s = dt_s;
mil_atc_hover_demo.t_final_s = t_final_s;
mil_atc_hover_demo.state0 = state0;
mil_atc_hover_demo.bridge_cfg = bridge_cfg;
mil_atc_hover_demo.final_truth = final_truth;
mil_atc_hover_demo.final_sensors = final_sensors;
mil_atc_hover_demo.final_estimator = final_estimator;
mil_atc_hover_demo.final_atc_cmd = final_atc_cmd;
mil_atc_hover_demo.final_diag = final_diag;
mil_atc_hover_demo.final_true_altitude_m = final_true_altitude_m;
mil_atc_hover_demo.final_estimated_altitude_m = final_estimated_altitude_m;

assignin('base', 'mil_atc_hover_demo', mil_atc_hover_demo);
...
if ~isa(signal, 'Simulink.SimulationData.Signal')
    error('uav:sl:run_mil_atc_hover:UnexpectedDatasetElement', ...
...
for k = 1:numel(field_names)
    ...
    elseif isa(field_value, 'timeseries')
        ...
        error('uav:sl:run_mil_atc_hover:UnexpectedLeafType', ...
...
elseif size(data, 2) == n_samples
    ...
    error('uav:sl:run_mil_atc_hover:UnexpectedTimeseriesShape', ...
