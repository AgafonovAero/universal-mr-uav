...
for case_index = 1:numel(cases)
    ...
    if start_result.process_alive
        ...
        if case_result.valid_rx_count <= 50 || case_result.response_tx_count <= 50 || case_result.last_frame_count <= 0
            case_result.failure_reason = "Базовый обмен для опыта не восстановлен: valid_rx_count=" ...
                + string(case_result.valid_rx_count) ...
                + ", response_tx_count=" + string(case_result.response_tx_count) ...
                + ", last_frame_count=" + string(case_result.last_frame_count) + ".";
        end
...
%LOCAL_START_SITL Запустить ArduPilot SITL прямой командой TASK-15.

[~, start_name, ~] = fileparts(start_log_path);
safe_name = regexprep(lower(start_name), '[^a-z0-9_]', '_');
wsl_log_path = "/home/oaleg/" + safe_name + "_arducopter.log";
wsl_pid_path = "/home/oaleg/" + safe_name + "_arducopter.pid";
wsl_exit_path = "/home/oaleg/" + safe_name + "_arducopter.exit";
wsl_wrapper_path = "/home/oaleg/" + safe_name + "_arducopter.wrapper";
...
if isfile(extra_defaults_path)
    file_info = dir(extra_defaults_path);
    if ~isempty(file_info) && file_info(1).bytes > 0
        defaults_arg = defaults_arg + "," + string(local_windows_to_wsl_path(extra_defaults_path));
    end
end

direct_command = sprintf([ ...
    'cd %s/ArduCopter && ' ...
...
tmp_ps1 = fullfile(tempdir, ['task17_launch_', safe_name, '.ps1']);
...
process_state = local_read_process_state(cfg.wsl_distro_name, wsl_pid_path, wsl_exit_path);

start_result = struct();
start_result.command_text = string(direct_command);
start_result.status_code = double(status_code);
start_result.output_text = string(output_text);
start_result.arducopter_pid = double(process_state.pid);
start_result.process_alive = logical(process_state.is_alive);
start_result.exit_code_text = string(process_state.exit_code_text);
end
...
%LOCAL_READ_PROCESS_STATE Прочитать состояние процесса arducopter внутри WSL.
...
state.is_alive = local_is_wsl_pid_alive(distro_name, state.pid);
end
...
command_text = sprintf( ...
    'wsl -d %s -- bash -lc "%s"', ...
    char(distro_name), ...
    strrep(char(string(bash_command)), '"', '\"'));
...
path_value = string(java.io.File(char(path_value)).getCanonicalPath());
drive_letter = lower(extractBefore(path_value, ":\"));
tail_value = extractAfter(path_value, ":\");
...
[move_ok, move_msg] = movefile(temp_path, path_value, 'f');
if ~move_ok
    if isfile(path_value)
        delete(path_value);
        [move_ok, move_msg] = movefile(temp_path, path_value, 'f');
...
if ~move_ok
    error( ...
        ...
        'Не удалось переместить MAT-файл в %s: %s', ...
        path_value, ...
        string(move_msg));
end
...
if ~move_ok
    error( ...
        ...
        'Не удалось переместить CSV-файл в %s: %s', ...
        path_value, ...
        string(move_msg));
end
