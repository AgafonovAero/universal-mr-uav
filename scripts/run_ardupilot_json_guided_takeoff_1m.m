%% RUN_ARDUPILOT_JSON_GUIDED_TAKEOFF_1M Выполнить первую команду GUIDED takeoff на 1 м.
% Назначение:
%   Поднимает рабочий стенд `ArduPilot JSON + MATLAB-модель`, переводит
%   аппарат в режим `GUIDED`, выполняет команду взведения и команду
%   `takeoff 1 m`, затем сохраняет историю ШИМ, команд винтомоторной
%   группы, тяги и высоты.
%
% Входы:
%   none
%
% Выходы:
%   task_23_guided_takeoff_1m - структура результата в base workspace
%
% Единицы измерения:
%   время - секунды;
%   высота - метры;
...
%
% Допущения:
%   Используется рабочий профиль TASK-22 с `SCHED_LOOP_RATE = 200`.

repo_root = fileparts(fileparts(mfilename('fullpath')));
logs_dir = fullfile(repo_root, 'artifacts', 'logs');
reports_dir = fullfile(repo_root, 'artifacts', 'reports');
log_path = fullfile(logs_dir, 'task_23_guided_takeoff_1m.txt');
csv_path = fullfile(reports_dir, 'task_23_guided_takeoff_1m.csv');
mat_path = fullfile(reports_dir, 'task_23_guided_takeoff_1m.mat');
...
if ~precalibration.calibration_ready
    ...
    uav.ardupilot.write_utf8_text_file(log_path, ...
        "TASK-23: GUIDED takeoff 1 m не выполнен." + newline + ...
        "Причина: " + string(result.failure_reason) + newline);
    error('uav:task23:guided:PrecalibrationFailed', ...
        'Не подтверждена простая калибровка акселерометра перед GUIDED takeoff: %s', char(result.failure_reason));
end

baseline = local_run_baseline(repo_root, logs_dir, parm_path, "task_23_guided");
if ~baseline.baseline_restored
    ...
    uav.ardupilot.write_utf8_text_file(log_path, ...
        "TASK-23: GUIDED takeoff 1 m не выполнен." + newline + ...
        "Причина: " + string(result.failure_reason) + newline);
    error('uav:task23:guided:BaselineFailed', ...
        'Не восстановлен устойчивый обмен перед GUIDED takeoff: %s', char(result.failure_reason));
end
...
seq_cfg.monitor_duration_s = 30.0;
...
result.guided_observed = any(strcmpi(strtrim(cellstr(history_table.mode)), 'GUIDED'));
result.arm_succeeded = logical(local_getfield_or(seq_result, 'arm_succeeded', false));
result.arm_command_ack = double(local_getfield_or(seq_result, 'arm_ack_result', nan));
result.takeoff_command_ack = double(local_getfield_or(seq_result, 'takeoff_ack_result', nan));
result.takeoff_accepted = isfinite(result.takeoff_command_ack) && result.takeoff_command_ack == 0;
...
result.altitude_changed = (result.max_altitude_m - history_table.altitude_m(1)) > 0.05;
...
if ~isfile(parm_path)
    error('uav:task23:guided:MissingParm', 'Не найден профиль TASK-23: %s', parm_path);
end
end
...
helper_log_path_win = [tempname, '_task23_guided_helper.log'];
launcher_path_win = [tempname, '_task23_guided_helper.ps1'];
...
lines = strings(0, 1);
lines(end + 1, 1) = "TASK-23: GUIDED takeoff 1 m";
lines(end + 1, 1) = "============================================================";
...
lines(end + 1, 1) = "valid_rx_count during takeoff: " + string(metrics.valid_rx_count);
lines(end + 1, 1) = "response_tx_count during takeoff: " + string(metrics.response_tx_count);
lines(end + 1, 1) = "last_frame_count: " + string(metrics.last_frame_count);
lines(end + 1, 1) = "GUIDED observed: " + local_bool_text(result.guided_observed);
lines(end + 1, 1) = "arm succeeded: " + local_bool_text(result.arm_succeeded);
lines(end + 1, 1) = "arm COMMAND_ACK: " + string(result.arm_command_ack);
lines(end + 1, 1) = "takeoff COMMAND_ACK: " + string(result.takeoff_command_ack);
lines(end + 1, 1) = "takeoff accepted: " + local_bool_text(result.takeoff_accepted);
...
lines(end + 1, 1) = "altitude changed upward: " + local_bool_text(result.altitude_changed);
