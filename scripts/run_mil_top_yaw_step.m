%% RUN_MIL_TOP_YAW_STEP Build and run the thin MIL shell on a yaw-step profile.
% Description:
%   Rebuilds the minimal Simulink shell, generates a deterministic yaw-step
...
%
% Outputs:
%   mil_top_yaw_step_demo - assigned in base workspace with parsed results
%
% Units:
...
cmd_profile = uav.sl.make_demo_command_profile('yaw_step', params, dt_s, t_final_s);
...
final_yaw_estimate_rad = final_estimator.euler_rpy_rad(3);
...
fprintf('Thin MIL yaw-step diagnostics:\n');
fprintf('  final yaw rate [rad/s]      : %.6f\n', final_yaw_rate_radps);
fprintf('  final yaw estimate [rad]    : %.6f\n', final_yaw_estimate_rad);
...
mil_top_yaw_step_demo = struct();
mil_top_yaw_step_demo.time_s = cmd_profile.time_s;
mil_top_yaw_step_demo.params = params;
mil_top_yaw_step_demo.dt_s = dt_s;
mil_top_yaw_step_demo.t_final_s = t_final_s;
mil_top_yaw_step_demo.state0 = state0;
mil_top_yaw_step_demo.cmd_profile = cmd_profile;
mil_top_yaw_step_demo.state_log = state_log;
mil_top_yaw_step_demo.estimator_log = estimator_log;
mil_top_yaw_step_demo.diag_log = diag_log;
mil_top_yaw_step_demo.final_yaw_rate_radps = final_yaw_rate_radps;
mil_top_yaw_step_demo.final_yaw_estimate_rad = final_yaw_estimate_rad;
mil_top_yaw_step_demo.final_altitude_m = final_altitude_m;
mil_top_yaw_step_demo.final_estimated_altitude_m = final_estimated_altitude_m;
mil_top_yaw_step_demo.final_true_quat_norm = final_true_quat_norm;
mil_top_yaw_step_demo.final_estimated_quat_norm = final_estimated_quat_norm;

assignin('base', 'mil_top_yaw_step_demo', mil_top_yaw_step_demo);
...
if ~isa(signal, 'Simulink.SimulationData.Signal')
    error('uav:sl:run_mil_top_yaw_step:UnexpectedDatasetElement', ...
        'Expected yout elements to be Simulink.SimulationData.Signal.');
end

values = signal.Values;
if ~isstruct(values)
    error('uav:sl:run_mil_top_yaw_step:UnexpectedSignalValues', ...
...
for k = 1:numel(field_names)
    ...
    if ~isa(field_value, 'timeseries')
        error('uav:sl:run_mil_top_yaw_step:UnexpectedLeafType', ...
...
error('uav:sl:run_mil_top_yaw_step:MissingTimeVector', ...
...
elseif size(data, 2) == n_samples
    ...
    error('uav:sl:run_mil_top_yaw_step:UnexpectedTimeseriesShape', ...
